...
import kotlinx.android.synthetic.main.activity_result.namaBurung
    ...
    private fun Result(){
        ...
        call.enqueue(object : retrofit2.Callback<Default>{
            ...
            override fun onResponse(call: Call<Default>?, response: Response<Default>?) {
                ...
                namaBurung.setText(response?.body()?.nm_brg)
                namaIlmiah.setText(response?.body()?.nm_ilm)
                spesies.setText(response?.body()?.spesies)
                makanan.setText(response?.body()?.makan)
                status.setText(response?.body()?.status)
                if (response?.body()?.nm_brg.isNullOrBlank()){
                    namaBurung.setText("Jenis Burung\nBelum tersedia")
                    namaIlmiah.setText("null")
